...
- **移动端适配**：≤768px 自动切换手机布局（底部 Tab 三视图：自选/图表/分析），原生下拉刷新可用
- **同花顺自选股**：设置页登录账号，左侧面板显示自选分组与股票（中文名+代码），点击切票并自动展开 K 线，30 分钟自动刷新
- **自选实时行情**：交易时段内按间隔（默认 5 秒，可调）批量拉取全部自选现价与当日涨跌（A 股+港股，500 只约 0.2 秒），红涨绿跌显示，支持按涨跌幅排序与「全部」聚合视图
- **全中文**：分析结果与飞书/PushPlus 推送中的专业术语全部中文化，界面术语悬停/点按显示解释；推送附带股票名称
