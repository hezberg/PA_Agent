"""Tests for order-opportunity detection (pure logic in services)."""
...
def test_confidence_gate_respected() -> None:
    decision = {"order_type": "限价单", "trade_confidence": "30"}
    assert not has_order_opportunity(decision, confidence_threshold=40)
    assert has_order_opportunity(decision, confidence_threshold=0)
    decision["trade_confidence"] = "85"
    assert has_order_opportunity(decision, confidence_threshold=40)
